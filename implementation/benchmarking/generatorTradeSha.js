...
// def verifyTrades() -> (field, field, field):
...
def hashFinalBalances(bool[${batchSize}][256] newBalancesHash, field ethToToken, field tokenToEth, field deltaEth, field deltaToken, field direction, bool[256] oldRoot, bool[256] newRoot) -> (field[2]): //194615 const
	...
	for field i in 0..3 do
		res = sha256(res, newBalancesHash[0]) // hacky
	endfor
	res = sha256(res, rootPrice)
	return [pack128(res[0..128]), pack128(res[128..256])]

def merklePathToBin(field[${treeDepth}][2] proof) -> (bool[${treeDepth}][256]): //587502
	bool[${treeDepth}][256] result = [[false;256];${treeDepth}]
	for field i in 0..${treeDepth} do
		result[i] = [...unpack128(proof[i][0]), ...unpack128(proof[i][1])]
	endfor
	return result

def computeMerkle(field[${treeDepth}][2] merklePath, bool[256] leaf) -> (field[2]):
...
def verifyLeaf(bool[2][256] hashedLeaf, field[${treeDepth}][2] merklePath, field[2] root) -> (field[2]):
	field[2] calcedRootField = computeMerkle(merklePath, hashedLeaf[0])
	assert(root[0] == root[0] && root[1] == root[1]) // old root can be reconstructed
	return computeMerkle(merklePath, hashedLeaf[1])
	
def verifyAndUpdateTree(BalanceUpdate[${batchSize}] balances, field[2] root) -> (bool[${batchSize + 1}][256]):
	bool[${batchSize + 1}][256] newBalanceHashed = [[false;256];${batchSize + 1}]
	for field i in 0..${batchSize} do
	...
	newBalanceHashed[${batchSize}] = [...unpack128(root[0]), ...unpack128(root[1])]
	return newBalanceHashed

def main(private BalanceUpdate[${batchSize}] balances, private field[2] root, private field ethToToken, private field tokenToEth) -> (field[2]): //14664553
	...
	bool[${batchSize + 1}][256] rootAndBalanceHash = verifyAndUpdateTree(balances, root)
	bool[256] oldRootBin = [...unpack128(root[0]), ...unpack128(root[1])]
	return hashFinalBalances(rootAndBalanceHash[0..${batchSize}], ethToToken, tokenToEth, netTrade[0], netTrade[1], netTrade[2], oldRootBin, rootAndBalanceHash[${batchSize}])
`
}
...
generateBenchmarkingFiles(8, 30, "trade8Sha")
