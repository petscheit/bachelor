...
class Benchmarker {
  ...
  async benchmarkMulti(n, distance = 3) {
    const leafIndexes = await this.generateLeafIndexes(n, distance)
    console.log(leafIndexes)
    const proofData = this.merkle.getMulti(leafIndexes)
    console.log("Number of hashes:", proofData[1].length)
  } 

  async manualBench(indices) {
    let merkle = new TransactorMerkle(Math.pow(2, 16))
    console.log(merkle.getMultiBenchmark(indices))
    // return[t, l, d, hashNumber[0], hashNumber[1], Math.round((hashNumber[0] / l) * 100) / 100]
  }
...
let benchmarker = new Benchmarker(16, 17, 2, 25, 10, 1000, 10)
// benchmarker.runBenchmark()
benchmarker.manualBench([0,2,28,30])
